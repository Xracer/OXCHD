...
void BattlescapeState::btnInventoryClick(Action *)
{
		...
		&& (_save->getSelectedUnit()->hasInventory() || _save->getDebugMode()))
	{
		...
		// Pushing inventory and UnitInfo states to the same screen as multistate
		MultiState *state = new MultiState;
		state->add(new InventoryState(!_save->getDebugMode(), this));
		state->add(new UnitInfoState(_save->getSelectedUnit(), this, false, false));
		// add more here

...
namespace OpenXcom
{
	...
	*/
	GraphsState::GraphsState() : _butRegionsOffset(0), _butCountriesOffset(0)
	{
		...
		// set up the grid
		_bg->drawRect(160, 95, 0, 250, gridColor); //change (125, 49, 188, 127) to (160, 95, 400, 250) x,y is location of bg x',y' is size of bg


		for (int grid = 0; grid != 5; ++grid)
		{
			for (int y = 100 + grid; y <= 225 + grid; y += 25) /// y=50 -> 100, y'=163 -> 325, y''=14 -> 25, y is location of grid y' is height is grid y'' is cell height

			{
				for (int x = 200 + grid; x <= 420 + grid; x += 35) //// 17 changed to 35 x = 126 -> 200; x is location of grid because the bg is linked to the grid
	...
	void GraphsState::drawRegionLines()
	{
		...
		for (size_t entry = 0; entry != _game->getSavedGame()->getRegions()->size(); ++entry)
		{
			...
			for (size_t iter = 0; iter != 12; ++iter)
			{
				...
				newLineVector.push_back(y);
				if (newLineVector.size() > 1 && _alien)
					_alienRegionLines.at(entry)->drawLine(x, y, x + 35, newLineVector.at(newLineVector.size() - 2), _regionToggles.at(entry)->_color + 4);
				else if (newLineVector.size() > 1)
					_xcomRegionLines.at(entry)->drawLine(x, y, x + 35, newLineVector.at(newLineVector.size() - 2), _regionToggles.at(entry)->_color + 4);
			}
